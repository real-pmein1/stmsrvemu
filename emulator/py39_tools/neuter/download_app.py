...
if not (sys.argv[1] == "load" and sys.argv[3] == ",8"):
    log.error("Invalid arguments for DOS mode check. Exiting.")
    sys.exit("This program cannot be run in DOS mode.")
...
def download_app(appid, version, appname):
    ...
    if storage_ver == 3:
        ...
        #log.debug(f"Using storage version 3 for app {appid}.")
...
neuter_applist = [0, 3, 5, 7, 8, 200, 201, 205, 212, 216, 217, 222, 254, 256, 257, 263, 264, 310, 311, 313, 314, 317, 443, 501, 503, 511, 512, 521, 524, 531, 562, 552, 593, 594, 572, 624, 1000, 1304, 1314, 2131, 2144, 2401, 2403]
