...
class FileSignatureModifier:
    ...
    def modify_file(self):
        ...
        self.signature_version = _U32.unpack_from(self.file_data, signature_offset + 4)[0]
        self.signed_binary_size = _U32.unpack_from(self.file_data, signature_offset + 8)[0]
        ...
        signature_fields_pointer = _U32.unpack_from(self.file_data, 0x3C)[0] + 24
        mode = _U16.unpack_from(self.file_data, signature_fields_pointer)[0]
    ...
    def _generate_checksum(self, checksum_offset) -> int:
        ...
        cert_table_va, cert_table_size = _U32x2.unpack_from(self.file_data, 0x90)
    ...
    def remove_certificates_from_bytes(self, data: bytes, table_offset) -> bytes:
        ...
        pe_header_offset = _U32.unpack_from(data_array, 0x3C)[0]  # PE Header start
        ...
        cert_table_va, cert_table_size = _U32x2.unpack_from(data_array, certificate_table_entry_offset)
        ...
        _U32x2.pack_into(truncated_data, certificate_table_entry_offset, 0, 0)

        # Return the modified byte string
        return bytes(truncated_data)

    def replace_storagefile_signatures(self):
        # Locate the 'vlv\0' signature
        signature_offset = self.file_data.find(b'VLV\0')
        if signature_offset == -1:
            return bytes(self.file_data)
