...
class ChatMemberInfo(MessageObject):
    def __init__(self, copy=None, input_stream=None):
        ...
        if copy is not None:
            ...
            self.raw_data = input_stream
            self.parse()
        else:
            self.set_SteamID(0)
            self.set_Permissions(0) # Assuming ClanPermission_nobody is 0
            self.set_Details(0)
    ...
    def set_SteamID(self, value):
        self.setValue("SteamID", value, KVS_TYPE_UINT64)

    def set_Permissions(self, value):
        self.setValue("Permissions", value, KVS_TYPE_INT)

    def set_Details(self, value):
        self.setValue("Details", value, KVS_TYPE_INT)
