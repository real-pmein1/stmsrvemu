...
def handle_ClientAppInfoRequest_obsolete(cmserver_obj, packet: CMPacket, client_obj: Client):
    ...
    cmserver_obj.log.info(f"{client_address}: (Obsolete) Application Info Request")

    msg = MsgClientAppInfoRequest()
    msg.is_obsolete = True
    msg.deserialize(data)

    # Pass full AppInfoRequest objects for section filtering (2010-2011 handler)
    return build_appinfo_response(client_obj, msg.appInfoRequests, True)
...
def handle_ClientAppInfoRequest(cmserver_obj, packet: CMPacket, client_obj: Client):
    ...
    # Pass full AppInfoRequest objects for section filtering (2010-2011 handler)
    return build_appinfo_response(client_obj, msg.appInfoRequests, False)
...
def handle_ClientAppInfoupdate(cmserver_obj, packet: CMPacket, client_obj: Client):
    ...
    cmserver_obj.log.info(f"{client_address} [handle_ClientAppInfoupdate] last change number: {last_change_number}, send change list: {send_change_list}")
    if bool(send_change_list):
        return build_appinfochanges_response(client_obj, last_change_number, bool(send_change_list))
    else:
        return []