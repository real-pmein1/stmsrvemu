...
class MsgClientCreateAcctResponse:
    """
    Create account response message (server to client).
    EMsg: 761 (ClientCreateAccountResponse)
    ...
    def deserialize(self, buffer: bytes, offset: int = 0) -> int:
        ...
        if len(buffer) < offset + self.BODY_SIZE:
            raise ValueError(
                f"Buffer too small for MsgClientCreateAcctResponse: need {self.BODY_SIZE} bytes"
            )
    ...
    def __repr__(self):
        return f"MsgClientCreateAcctResponse(result={self.result})"
