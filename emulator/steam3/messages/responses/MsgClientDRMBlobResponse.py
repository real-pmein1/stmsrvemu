...
class MsgClientDRMBlobResponse:
    """
    DRM blob response message (server to client).
    EMsg: 897 (ClientDRMBlobResponse)
    ...
    def deserialize(self, buffer: bytes, offset: int = 0) -> int:
        ...
        if len(buffer) < offset + self.BODY_SIZE:
            raise ValueError(
                f"Buffer too small for MsgClientDRMBlobResponse: need {self.BODY_SIZE} bytes"
            )
    ...
    def to_clientmsg(self) -> CMResponse:
        ...
        """
        packet = CMResponse(eMsgID=EMsg.ClientDRMBlobResponse, client_obj=self.client_obj)
    ...
    def __repr__(self):
        return f"MsgClientDRMBlobResponse(result={self.result})"
