...
class MsgGSCreateItemResponse:
    """
    MsgGSCreateItemResponse - Response to GSCreateItem request.

    Structure from IDA analysis (MsgGSCreateItemResponse_t):
        - m_eResult: uint32 - Result code (EResult)
        - m_ulSteamID: uint64 - Steam ID of the user
        - m_ulItemID: uint64 - The created item ID (0 if failed)
        - CPersistentItem data (optional, only if successful)

    Sent by CM server in response to GSCreateItem (EMsg 912).
    Response is EMsg 913 (GSCreateItemResponse).
    """

    def __init__(self, result=1, steam_id=0, item_id=0, item=None):
        self.result = result  # EResult value (1 = OK)
        self.steam_id = steam_id  # User's Steam ID
        self.item_id = item_id  # Created item ID (0 if failed)
        self.item = item  # Optional: Full item data for successful creates

    def deserialize(self, byte_buffer):
        """Deserializes the byte buffer into a MsgGSCreateItemResponse object."""
        ...
        if self.result == 1 and len(remaining) >= 20:
    ...
    def serialize(self):
        """Serializes the MsgGSCreateItemResponse object into a byte buffer."""
        stream = BytesIO()
        ...
        # Optionally write item data for successful creates
    ...
    def __repr__(self):
        return (f"MsgGSCreateItemResponse(result={self.result}, "
