...
class MsgClientEmailChangeResponse_DEPRECATED:
    """
    Response to deprecated email change request.
    EMsg: 891 (ClientEmailChangeResponse)
    ...
    def deserialize(self, buffer: bytes, offset: int = 0) -> int:
        ...
        if len(buffer) < offset + self.BODY_SIZE:
            raise ValueError(
                f"Buffer too small for MsgClientEmailChangeResponse_DEPRECATED: need {self.BODY_SIZE} bytes"
            )
    ...
    def to_clientmsg(self) -> CMResponse:
        ...
        """
        packet = CMResponse(eMsgID=EMsg.ClientEmailChangeResponse, client_obj=self.client_obj)
    ...
    def __repr__(self):
        return f"MsgClientEmailChangeResponse_DEPRECATED(result={self.result})"
