...
class MsgClientPasswordChangeResponse:
    """
    Response to password change request.
    EMsg: 805 (ClientPasswordChangeResponse)
    ...
    def deserialize(self, buffer: bytes, offset: int = 0) -> int:
        ...
        if len(buffer) < offset + self.BODY_SIZE:
            raise ValueError(
                f"Buffer too small for MsgClientPasswordChangeResponse: need {self.BODY_SIZE} bytes"
            )
    ...
    def to_clientmsg(self) -> CMResponse:
        ...
        """
        packet = CMResponse(eMsgID=EMsg.ClientPasswordChangeResponse, client_obj=self.client_obj)
    ...
    def __repr__(self):
        return f"MsgClientPasswordChangeResponse(result={self.result})"
